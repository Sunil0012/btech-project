import { teacherClassroomSupabase } from "@/integrations/supabase/teacher-client";
import { studentTeacherSupabase } from "@/integrations/supabase/student-client";
...
export async function uploadAssignmentFile(
  ...
  // Upload to the classroom schema - teachers only.
...
export async function fetchAssignmentFiles(assignmentId: string) {
  const teacherResult = await teacherClassroomSupabase
    .from("assignment_files")
    .select("id, file_name, file_type, file_size, created_at")
    .eq("assignment_id", assignmentId)
    .order("created_at", { ascending: false });

  const result = teacherResult.error
    ? await (studentTeacherSupabase as any)
        .from("assignment_files")
        .select("id, file_name, file_type, file_size, created_at")
        .eq("assignment_id", assignmentId)
        .order("created_at", { ascending: false })
  ...
  if (result.error) {
    console.error("Failed to fetch assignment files:", result.error);
    return [];
  }
...
export async function deleteAssignmentFile(fileId: string) {
  // Delete from the classroom schema - teachers only.
