% Compile with: pdflatex -interaction=nonstopmode -halt-on-error report.tex
\documentclass[sigconf,authordraft,twoside]{acmart}
%%
...
\hypersetup{
  ...
  pdfauthor={Khethavath Sunil Naik},
}
...
\newcommand{\tech}[1]{\textcolor{brandblue}{\nolinkurl{#1}}}
\newcommand{\project}{GATEWay}
\newcommand{\formula}[1]{\textbf{\textit{#1}}}
\newcommand{\reporttitle}{GATEWay: An Intelligent GATE DA Preparation Platform}
\newcommand{\reportsubtitle}{With Advanced Recommendation Engines, Analytics, and Predictive Scoring}
\newcommand{\reportauthorname}{Khethavath Sunil Naik \\ Dosapati Ashirvadhan}
\newcommand{\reportrollnumber}{12341170}
\newcommand{\reportprepared}{April 2026}

\setcopyright{none}
\acmConference[B.Tech Project Report]{B.Tech Project Report}{\reportprepared}{India}
\acmBooktitle{B.Tech Project Report}
\acmDOI{}
\acmISBN{}
\settopmatter{printacmref=false}
\renewcommand\footnotetextcopyrightpermission[1]{}

\title[\project{}]{\reporttitle}
\subtitle{\reportsubtitle}
\author{\reportauthorname}
\authornote{Roll Number: 12341170}
\affiliation{%
  \institution{B.Tech Project Report}
}
\renewcommand{\shortauthors}{\reportauthorname}
...
\keywords{GATE DA, adaptive learning, recommendation systems, educational analytics, score prediction}
...
\begin{tikzpicture}[node distance=1.3cm and 1.8cm, scale=0.95, every node/.style={transform shape}]
  % Define node styles for the full-width architecture overview
  \tikzset{
    flowbox/.style={
      rectangle,
      rounded corners=5pt,
...
\centering
\small
\begin{tabularx}{\columnwidth}{>{\raggedright\arraybackslash}p{0.24\columnwidth} >{\raggedright\arraybackslash}p{0.17\columnwidth} X}
\toprule
...
\item $h(q')$ is a \textbf{heuristic score} combining:
   ...
   Typically:
   \begin{equation}
   h(q') = 1.0 - \frac{w_1 \cdot d_{\text{diff}} + w_2 \cdot d_{\text{elo}}}{C_{\text{norm}}} + \beta_{\text{weak}} \cdot \mathbb{1}[q' \in \text{weak\_topics}] 
   \ 



   
   + \beta_{\text{type}} \cdot \mathbb{1}[\text{type}(q') \neq \text{type}(q)]
   \end{equation}
...
\centering
\footnotesize
\begin{tabularx}{\columnwidth}{>{\raggedright\arraybackslash}p{0.29\columnwidth} X}
\toprule
\textbf{Condition} & \textbf{Generated suggestion} \\
\midrule
Weak subjects available & Focus on $w_1$ today; you still have $(100 - \text{acc}_{w_1})\%$ room for improvement. \\
Streak $> 5$ & Great momentum. Tackle harder problems in $s_1$ today. \\
Otherwise & Review fundamentals in $w_1$ to build confidence. \\
\bottomrule
\end{tabularx}
\caption{Daily study suggestion rules used by the deterministic coach.}
\end{table}
...
Suggested action: Publish a 5-question probability\\
assignment.\\
...
This captures improvements and declines up to $\pm 15$ percentage points.
...
\toprule
\textbf{Expected score band} & \textbf{Generated guidance} \\
\midrule
$\text{Expected} \geq 75$ & Excellent. Expect a GATE score around 80--90 and focus on maintaining consistency across all topics. \\
$60 \leq \text{Expected} < 75$ & Good progress. Expect a score around 60--70, strengthen weak topics, and increase mock-test frequency. \\
$\text{Expected} < 60$ & Preparation is on track for 40--60. Increase study intensity, prioritise weak areas, and attempt one mock test per week. \\
\bottomrule
\end{tabularx}
\caption{Guidance text produced by the score predictor.}
\end{table}
...
\begin{quote}
\footnotesize
Good progress. Expected GATE score: 76--86 (best estimate: 78).
Consistency is strong across topics. Recent acceleration (+7\%) is
encouraging, so maintain this pace with daily practice on weak areas.
\end{quote}
